...
def find_graph_files():
    ...
    # Look for files with pattern unseed_G*.edgelist
    files = sorted(glob.glob("unseed_G*.edgelist"))
    
    if len(files) < 2:
        raise FileNotFoundError("Could not find at least two unseed_G*.edgelist files in the current directory")
